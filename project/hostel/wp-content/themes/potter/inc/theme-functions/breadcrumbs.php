...
/**
 * Shows a breadcrumb for all types of pages.  This is a wrapper function for the POTTER_Breadcrumbs class,
 * which should be used in theme templates.
 *
 * @since  0.1.0
 * @access public
 * @param  array $args Arguments to pass to POTTER_Breadcrumbs.
 * @return void
 */
function potter_do_breadcrumbs( $args = array() ) {
	...
	if ( ! is_object( $breadcrumb ) )
		$breadcrumb = new POTTER_Breadcrumbs( $args );

	return $breadcrumb->trail();

}

function potter_breadcrumbs_header() {

		potter_do_breadcrumbs();

}
add_action( 'potter_breadcrumbhead', 'potter_breadcrumbs_header' );


/**
 * Add Breadcrumbs to Content
 */
/*function potter_breadcrumbs_content() {
...
add_action( 'potter_inner_content_open', 'potter_breadcrumbs_content' );
*/
/**
...
add_action( 'potter_after_header', 'potter_breadcrumbs_header' );
*/
/**
...
class POTTER_Breadcrumbs {
	...
	/**
	 * Sets up the breadcrumb trail properties.  Calls the `POTTER_Breadcrumbs::add_items()` method
	 ...
	 *     @type array     $labels         Text labels. @see POTTER_Breadcrumbs::set_labels()
	 *     @type array     $post_taxonomy  Taxonomies to use for post types. @see POTTER_Breadcrumbs::set_post_taxonomy()
	...
	public function __construct( $args = array() ) {
		...
		$defaults = array(
			...
			'list_tag'        => 'ul',
			'item_tag'        => 'li',
			'separator'		  => apply_filters( 'potter_breadcrumbs_separator', '/' ),
			'show_on_front'   => true,
			'network'         => false,
			'show_title'      => true,
			'labels'          => array(),
	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			$breadcrumb .= sprintf(
				'<%s class="potter-breadcrumbs" itemscope="itemscope" itemtype="https://schema.org/BreadcrumbList">',
			...
			foreach ( $this->items as $item ) {
				...
				$item_class = 'potter-breadcrumb';

				if ( 1 === $item_position && 1 < $item_count ) {
					$item_class .= ' potter-breadcrumb-first';
					$separator = $this->args['separator'];

				} elseif ( $item_count === $item_position ) {
					$item_class .= ' potter-breadcrumb-last';
					$separator = '';
				}
			...
			$breadcrumb = sprintf(
				'<%1$s aria-label="%2$s" class="potter-breadcrumbs-wrapper" itemprop="breadcrumb">%3$s%4$s%5$s</%1$s>',
		...
		$breadcrumb = apply_filters( 'potter_breadcrumbs', $breadcrumb, $this->args );
		...
		if( get_theme_mod( 'breadcrumbs_position' ) === 'header' ) {
			...
			$output  = '<div class="potter-breadcrumbs-container">';
			$output .= $breadcrumb;
			$output .= '</div>';
	...
	protected function set_labels() {
		...
		$defaults = array(
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'potter' ),
			'home'                => esc_html__( 'Home', 'potter' ),
			'error_404'           => esc_html__( '404 Not Found', 'potter' ),
			'archives'            => esc_html__( 'Archives', 'potter' ),
			// Translators: %s is the search query.
			'search'              => esc_html__( 'Search results for: %s', 'potter' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s', 'potter' ),
			// Translators: %s is the page number.
			'paged_comments'      => esc_html__( 'Comment Page %s', 'potter' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s', 'potter' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s', 'potter' ),
